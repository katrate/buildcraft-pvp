...
describe('findRankedGroup (rank window cap)', () => {
  ...
  it('matches players of the same rank band', () => {
    ...
    expect(group?.length).toBe(4); // 4 bronze (tier 1)
  });

  it('matches adjacent bands (Bronze + Silver together)', () => {
    const group = findRankedGroup(tiers([1000, 1020, 1120, 1150]), 2);
    expect(group?.length).toBe(4); // 3 bronze + 1 silver — all inside the window
  });
  ...
  it('a silver player bridges bronze and gold by matching one of them', () => {
    // bronze(1) + silver(2) + gold(3), 1v1 -> picks the 2-player window; tie
  ...
  it('pairs adjacent high bands: platinum+diamond and divine+supreme', () => {
    expect(findRankedGroup(tiers([1500, 1600]), 1)?.length).toBe(2); // platinum + diamond
    expect(findRankedGroup(tiers([1800, 1900]), 1)?.length).toBe(2); // divine + supreme
  });
  ...
  it('prefers the largest compatible window', () => {
    // 3 bronze + 1 silver + 2 gold: window {1,2} = 4 and {2,3} = 3; lower band wins
    ...
    expect(picked.every((t) => t === 1 || t === 2)).toBe(true); // bronze + silver only
  });

  it('widens to ±2 bands when the window is enlarged', () => {
    // bronze(1) + gold(3): blocked at ±1, allowed at ±2
    expect(findRankedGroup(tiers([1000, 1400]), 1)).toBeNull();
    expect(findRankedGroup(tiers([1000, 1400]), 1, 2)?.length).toBe(2);
    // bronze(1) + platinum(4) = spread 3: still blocked even widened
    expect(findRankedGroup(tiers([1000, 1500]), 1, 2)).toBeNull();
    // bronze(1) + divine(6) = spread 5: still blocked even widened
    expect(findRankedGroup(tiers([1000, 1800]), 1, 2)).toBeNull();
    // silver(2) + platinum(4) = spread 2: now allowed when widened
    expect(findRankedGroup(tiers([1200, 1500]), 1, 2)?.length).toBe(2);
  });
...
describe('party queue units', () => {
  ...
  it('buildUnits groups party members into one unit anchored on the creator tier', () => {
    ...
    expect(units[1].tier).toBe(2); // 1200 -> Silver (tier 2)
  });
  ...
  it('findRankedPartyGroup anchors opponents on the party creator tier', () => {
    // iron-tier party (2) + 2 same-tier solos -> match, party whole on one team
  ...
  it('iron party matches bronze opponents (enemy matched around creator rank)', () => {
    const res = findRankedPartyGroup(
      [
        { id: 'p', tier: 0, size: 2, party: true },
  ...
  it('silver opponents are out of the iron party window (±1)', () => {
    expect(
